...
client.on('messageCreate', async message => {
    ...
    // === LOCK/UNLOCK BOT COMMANDS ===
    if (command === 'lock' && args[0] === 'bot' && args[1] === 'cmds') {
        if (!isAuthorized(message)) {
        ...
        try {
            ...
            if (targetRole) {
                // Lock bot commands for specific role
                await message.channel.permissionOverwrites.edit(targetRole, {
                    SendMessages: false
                }, `Bot commands locked for ${targetRole.name} by ${message.author.username}`);

                const lockEmbed = new EmbedBuilder()
                    .setColor('#FF0000')
                    .setTitle('🔒 Bot Commands Locked')
                    .setDescription(`Successfully locked bot commands in this channel`)
                    .addFields(
                        ...
                        { name: '🚫 Effect', value: 'Role members cannot send messages in this channel', inline: false }
                    )
                    .setFooter({ text: 'Bot Command Lock System' })
                    .setTimestamp();

                await message.reply({ embeds: [lockEmbed] });
                await sendLogMessage(message.guild, lockEmbed);
            } else {
                // Lock bot commands for everyone
                await message.channel.permissionOverwrites.edit(message.guild.roles.everyone, {
                    SendMessages: false
                }, `Bot commands locked for everyone by ${message.author.username}`);

                const lockEmbed = new EmbedBuilder()
                    .setColor('#FF0000')
                    .setTitle('🔒 Bot Commands Locked')
                    .setDescription(`Successfully locked bot commands in this channel for everyone`)
                    .addFields(
                        { name: '📺 Channel', value: `${message.channel}`, inline: true },
                        { name: '👑 Locked By', value: `${message.author.username}`, inline: true },
                        { name: '⏰ Locked At', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true },
                        { name: '🚫 Effect', value: '@everyone cannot send messages in this channel', inline: false }
                    )
                    .setFooter({ text: 'Bot Command Lock System' })
                    .setTimestamp();
            ...
            await message.reply('❌ Failed to lock bot commands. Make sure I have the Manage Channels permission.');
        }
        return;
    }

    if (command === 'unlock' && args[0] === 'bot' && args[1] === 'cmds') {
        if (!isAuthorized(message)) {
            return message.reply('❌ You are not authorized to use this command.');
        }
        ...
        try {
            ...
            if (targetRole) {
                // Unlock bot commands for specific role
                await message.channel.permissionOverwrites.edit(targetRole, {
                    SendMessages: null
                }, `Bot commands unlocked for ${targetRole.name} by ${message.author.username}`);

                const unlockEmbed = new EmbedBuilder()
                    .setColor('#00FF00')
                    .setTitle('🔓 Bot Commands Unlocked')
                    .setDescription(`Successfully unlocked bot commands in this channel`)
                    .addFields(
                        { name: '🎭 Role', value: `${targetRole}`, inline: true },
                        { name: '📺 Channel', value: `${message.channel}`, inline: true },
                        ...
                        { name: '✅ Effect', value: 'Role members can now send messages in this channel', inline: false }
                    )
                    .setFooter({ text: 'Bot Command Lock System' })
                    .setTimestamp();

                await message.reply({ embeds: [unlockEmbed] });
                await sendLogMessage(message.guild, unlockEmbed);
            } else {
                // Unlock bot commands for everyone
                await message.channel.permissionOverwrites.edit(message.guild.roles.everyone, {
                    SendMessages: null
                }, `Bot commands unlocked for everyone by ${message.author.username}`);

                const unlockEmbed = new EmbedBuilder()
                    .setColor('#00FF00')
                    .setTitle('🔓 Bot Commands Unlocked')
                    .setDescription(`Successfully unlocked bot commands in this channel for everyone`)
                    .addFields(
                        { name: '📺 Channel', value: `${message.channel}`, inline: true },
                        { name: '👑 Unlocked By', value: `${message.author.username}`, inline: true },
                        { name: '⏰ Unlocked At', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true },
                        { name: '✅ Effect', value: '@everyone can now send messages in this channel', inline: false }
                    )
                    .setFooter({ text: 'Bot Command Lock System' })
                    .setTimestamp();
            ...
            await message.reply('❌ Failed to unlock bot commands. Make sure I have the Manage Channels permission.');
        }
        return;
    }
